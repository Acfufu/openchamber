...
- Phones and tablets share one navigation model: a sessions drawer/sidebar on the left, the workspace drawer (Changes / Files / Terminal / Notes / Repo Wiki / MCP) on the right, and no overflow menu. Tablets differ only in that the sessions list is a resizable persistent sidebar and the header dropdowns are anchored popovers.
...
- The Repo Wiki workspace tab renders the shared panel read-only: it shows the active project's generated guide with no generate/retry/stop/delete controls, and tapping a source reference opens the file in the Files tab at the cited line. A navigation that starts from a drawer tab is remembered, so Android back returns to it before the drawer closes.
